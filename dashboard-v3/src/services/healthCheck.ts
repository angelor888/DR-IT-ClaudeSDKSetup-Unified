...
class HealthCheckService {
  ...
  async getMockHealthData(): Promise<HealthCheckResult> {
    ...
    const services: ServiceHealth[] = [
      ...
      {
        name: 'PostgreSQL',
        endpoint: import.meta.env.VITE_POSTGRES_HOST || 'localhost:5432',
        status: import.meta.env.VITE_POSTGRES_PASSWORD ? 'connected' : 'disconnected',
        lastChecked: new Date(),
        responseTime: 150,
        errorMessage: import.meta.env.VITE_POSTGRES_PASSWORD ? undefined : 'Database credentials not configured',
        capabilities: ['customers', 'jobs', 'invoices', 'analytics', 'reporting'],
      },
      {
        name: 'Redis Cache',
        endpoint: (import.meta.env.VITE_REDIS_HOST || 'localhost') + ':' + (import.meta.env.VITE_REDIS_PORT || '6379'),
        status: 'connected',
        lastChecked: new Date(),
        responseTime: 50,
        capabilities: ['session_management', 'caching', 'rate_limiting', 'analytics_cache'],
      },
      {
        name: 'Matterport',
        endpoint: 'https://api.matterport.com/api/v1',
        status: import.meta.env.VITE_MATTERPORT_API_KEY ? 'connected' : 'disconnected',
        lastChecked: new Date(),
        responseTime: 300,
        errorMessage: import.meta.env.VITE_MATTERPORT_API_KEY ? undefined : 'API key not configured',
        capabilities: ['3d_scanning', 'model_management', 'measurements', 'sharing', 'analytics'],
      },
      {
        name: 'Browser Automation',
        endpoint: 'Puppeteer/Playwright',
        status: 'connected',
        lastChecked: new Date(),
        responseTime: 800,
        capabilities: ['screenshots', 'pdf_generation', 'web_scraping', 'form_automation', 'monitoring'],
      },
    ];
