...
/**
 * Script to send daily progress report to #it-report Slack channel
 * Usage: ts-node scripts/send-daily-report.ts
 */

import { postToItReport } from '../src/utils/slack-reporter';
...
async function main() {
  console.log('Sending daily progress report to #it-report Slack channel...');
  
  try {
    const success = await postToItReport(DAILY_REPORT);
    
    if (success) {
      console.log('✅ Daily report sent successfully to #it-report channel');
    } else {
      console.log('❌ Failed to send daily report to Slack');
      process.exit(1);
    }
  } catch (error) {
