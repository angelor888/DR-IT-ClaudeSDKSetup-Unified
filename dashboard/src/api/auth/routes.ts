...
import { registerValidation, updateUserValidation, resetPasswordValidation } from './validation';
import {
  authLimiter,
...
router.post(
  ...
  async (req: Request, res: Response<AuthResponse>) => {
    try {
      ...
      await getFirestore().collection('users').doc(userRecord.uid).set(userDoc);
