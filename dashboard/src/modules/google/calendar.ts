...
export class CalendarService {
  ...
  // Delete event
  async deleteEvent(eventId: string, calendarId?: string, userId?: string): Promise<void> {
    try {
      const calendar = await this.getCalendarClient(userId);
  ...
  async importFromJobber(
    ...
    const attendees = jobberJob.client?.email ? [{ email: jobberJob.client.email }] : undefined;
