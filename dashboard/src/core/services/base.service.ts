...
import { ServiceError, ServiceUnavailableError, ServiceTimeoutError } from '../errors/service.error';
...
export abstract class BaseService extends EventEmitter {
  ...
  private setupAuth(): void {
    ...
    switch (this.options.auth.type) {
      case 'bearer':
        this.axios.defaults.headers.common['Authorization'] = `Bearer ${this.options.auth.credentials}`;
        break;
      case 'basic':
  ...
  protected patch<T = any>(url: string, data?: any, config?: AxiosRequestConfig): Promise<AxiosResponse<T>> {
